...
function newWebsocket() {
    if (window.location.protocol == "https:") {
        // return new ReconnectingWebSocket('wss://' + window.location.host + '/socket', null, {debug: false, reconnectInterval: 5000});
        return new WebSocket('wss://' + window.location.host + '/socket');
    }else {
        // return new ReconnectingWebSocket('ws://' + window.location.host + '/socket', null, {debug: false, reconnectInterval: 5000});
...
function SocketTransport() {
	...
	function send(m) {
	    if (wsEnable) {
            ...
            outputs[m.Id]({Kind: 'stderr', Body: '\nConnection Already Closed.'});

            // outputs[m.Id]({Kind: 'abort'});
	        // throw Error("closed");
        }
	}
